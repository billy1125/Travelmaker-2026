...
- **`hotel/`（旅宿指南）**：一間旅宿一個檔案，記錄訂房事實（確認碼、PIN、日期、房型、人數、房價、取消政策）、館內設施與時間、房內設備、特殊規定、周邊生活機能、本次行程使用規劃，以及**四個情境的英日文聯絡信件**（行前確認／入住與鑰匙／抵達時間變更／行李寄放與其他需求）與**館方回信原文**（`## 飯店回信`，依收信日分節照抄）。訂房事實一律以 `assets/` 的訂房確認單 PDF 為準，網路查到的資料不覆蓋它。
...
**所有片假名**都適用，包含地名、景點、店家、設施、列車、業者名稱，以及一般的普通名詞。依下列順序處理：

1. **有官方漢字或英文名稱者，直接用官方寫法**，不寫片假名。例：`イオンスタイル尾道` → `AEON STYLE 尾道`、`スマイルホテル岡山` → `Smile Hotel 岡山`、`マリンライナー` → `Marine Liner`、`トモテツ` → `鞆鉄道`。
2. **沒有官方漢字或英文名稱者，寫成「翻譯（片假名）」**，翻譯在前、片假名在括號內，且**每次出現都用完整形式**，不因前文已出現過就簡寫。例：`因島巴士轉運站（因島バスセンター）`、`岡電博物館（おかでんミュージアム）`、`Alba Town（アルバタウン）`。
3. **已經身處全形括號內時改用「翻譯／片假名」**，避免巢狀括號。例：`0848-22-3911（Shimanami Japan／しまなみジャパン）`。
4. **車站、電停與巴士站牌名稱視為一個整體**，不在名稱中間插入括號。例：`東山・岡電博物館駅（東山・おかでんミュージアム駅）`，不寫成 `東山・岡電博物館（おかでんミュージアム）駅`。
5. **普通名詞直接寫中文，不加括號標注**，因為中文本來就有對應詞。例：`タクシー` → `計程車`、`サウナ` → `三溫暖`、`ジェットバス` → `按摩浴缸`、`ダイヤ` → `班表`、`路線バス` → `路線巴士`、`チャージ機` → `加值機`。

不套用此規則的情形：

- **URL 內的字串**（尤其 Google Maps 的 `query=` 參數）一律保留片假名，改成英文會查不到正確地點
- **旅宿指南的「日文原名」欄位**與**要寄給館方的日文信件**，功能就是提供日文原文
- **引用來源的日文網頁標題與網站名稱**（`參考來源` 一節的連結標題、`website_abstract.md` 的 `### [...]` 來源標題），例如 `コインロッカーのしおり`、`JR おでかけネット`、`備後カメラ部`；改寫會與原站對不上
- **日文商品名、料理名與店名**，例如 `はっさくソフトクリーム`、`デミカツ丼`、`尾道ラーメン 東珍康`
- **現場標示就是片假名的設施正式名稱**，例如 `みどりの券売機プラス`（與 `みどりの窓口` 成對出現）、`ねこのてステーション`
